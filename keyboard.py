...
from config import KEYBOARD_EVENT_PORT, KEYBOARD_EVENT_ENDPOINT
from dataclass import KeyboardEventTimestamp
# Listen to keyboard events
from pynput import keyboard
from kill import kill_and_run

latest_keypress_timestamp = KeyboardEventTimestamp(timestamp=0)

def update_latest_keypress_event_timestamp():
    # global latest_keypress_timestamp
    latest_keypress_timestamp['timestamp']=time.time()
...
async def get_keyboard():
    # global latest_keypress_timestamp
    return latest_keypress_timestamp
...
def main():
    ...
    kill_and_run(app, port=KEYBOARD_EVENT_PORT, application_name='keyboard listener')
